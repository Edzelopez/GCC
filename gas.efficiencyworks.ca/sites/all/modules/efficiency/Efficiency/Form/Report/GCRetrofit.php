...
class Efficiency_Form_Report_GCRetrofit extends CRM_Report_Form {
    ...
    function __construct( ) {
        ...
        foreach ( array('gcc_applicant', 'gcc_measures_other', 
            ...
            foreach ( $this->_columns[$table]['filters'] as $cfId => $cfVal ) {
                if ( !in_array($cfVal['name'], array('file_identifier', 'retrofit_invoiced', 'retrofit_completed' )) ) {
    ...
    function orderBy( ) {
        $this->_orderBy = "ORDER BY {$this->_aliases['gcc_applicant']}.file_identifier ASC";
    }
